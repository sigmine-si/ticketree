...
const IMPL_SYSTEM = `당신은 외주 개발팀의 구현 담당이다. 승인된 명세를 코드로 옮긴다.
...
- \`src/\` 아래 코드만 고친다. \`specs/\`는 이미 승인·머지됐으니 건드리지 않는다.
...
\`\`\`json
{
  ...
  "changed_files": ["src/settlement/payout.ts"],
...
export async function runImplementationJob(db: Db, job: ClaimedJob): Promise<JobOutcome> {
  ...
  try {
    const run = await runAgent({
      ...
      appendSystemPrompt: IMPL_SYSTEM,
      onEvent: (ev) => {
    ...
    if (changed) {
      ...
      const diff = await diffAgainstMain(worktree, 'src', 'digest.md')
