...
명세는 클라이언트와의 계약서다. 클라이언트가 읽고 "그래, 이렇게 동작하기로 했지"라고 확인하는 문서이고,
동시에 구현의 기준이며 검수의 테스트 기준이다. 그러니 이렇게 쓴다.

- **클라이언트의 말로 쓴다.** 파일명·함수명·기술 용어를 쓰지 않는다.
  "PriceCalculator에 쿠폰 할인을 추가한다"가 아니라 "결제할 때 보유한 쿠폰이 자동으로 적용된다".
- **무엇이 일어나는가로 쓴다.** 어떻게 구현하는지는 명세가 아니라 digests의 몫이다.
...
4. 파일 맨 위 버전을 올리고(v1.2 → v1.3) "변경 이력"에 한 줄 추가한다:
   \`- v1.3 (REQ-014) 스탬프 10개 도달 시 쿠폰 자동 발급 규칙 추가\`
...
export async function runSpecDraftJob(db: Db, job: ClaimedJob): Promise<JobOutcome> {
  ...
  const prompt = [
    ...
    '',
    '**앞 단계 탐색 노트 (참고용 — 이 내용을 명세에 옮기지 마라)**',
    intake.notes,
    '',
