...
0. \`specs/\` 바로 아래 문서를 먼저 읽는다 — product(제품 정의)·overview(전체 흐름)·
   screens(화면)·usecases(유스케이스와 비정상 경로). 이 변경이 "안 만드는 것"에
   걸리면 진행하지 말고 discrepancies에 적는다.
   화면이 바뀌면 screens.md를, 새 비정상 경로가 생기면 usecases.md를 함께 고친다.
...
- **같은 사실을 두 곳에 적지 않는다.** 기능 명세에 쓴 것을 \`overview.md\`에 또 쓰지 않는다.
  overview는 어느 기능도 혼자 소유하지 않는 것(상태 목록·레인·큐·데이터 모델)만 갖는다.
  이미 양쪽에 있으면 overview 쪽을 링크로 바꾼다 — 맞추지 말고 없앤다.
