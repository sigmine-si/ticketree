...
### 허브 repo 패턴 (멀티 repo 지원)

프로젝트(클라이언트가 인식하는 제품)와 repo(코드 저장 단위)를 분리한다. Spec은 repo가 아니라 프로젝트에 속하므로, 프로젝트당 우리 조직 소유의 허브 repo를 둔다. 단일 repo 프로젝트도 같은 구조로 통일한다.

```
workspaces/cafe-app-hub/         # 허브 repo 클론 (git 관리 대상: 명세·규칙·설정)
├── CLAUDE.md                    # 플랫폼 규칙 + 프로젝트 규칙
├── repos.yml                    # 코드 repo 목록·역할·의존 순서·배포 어댑터
├── specs/features/*.md          # 기능 단위 명세 (repo 단위 아님)
├── digests/{repo}.md            # repo별 코드 지도 (구현 job이 갱신)
├── .env                         # 프로젝트 공통 env (gitignored)
├── repos/                       # (gitignored) 코드 repo 클론, main
│   ├── api/                     #   각 repo의 .env도 여기 (통상 개발 환경 그대로)
│   └── web/
└── work/                        # (gitignored) 구현 작업장
    └── REQ-014/
        ├── api/                 #   git worktree, 브랜치 dev/REQ-014
        └── web/
```

Spec PR은 허브 repo의 PR이다. 클라이언트 코드 repo에는 기능 브랜치만 만들며 플랫폼 파일을 커밋하지 않는다. 요청 하나 = Spec PR 하나 + 코드 PR N개(repo별). 배포 승인 시 러너가 repos.yml의 의존 순서대로 머지·배포한다.
...
-- 사용자 (클라이언트는 초대링크+PIN, 관리자는 GitHub OAuth — §16-1)
users (
...
1순위 — 구독 인증의 정책·한도 리스크. 완화: 인증 추상화(전환 = 환경 변수 교체), Anthropic 정책 확인, 관리자 승인 게이트로 "운영자가 실행 주체"인 구조 유지. 2순위 — 대화 레인 응답 속도(2~3분 예산이 제품 품질). 완화: 레인 분리, digest 기반 탐색 단축, 탈출구 UX. 3순위 — 멀티 repo preview. 완화: "api는 스테이징 선배포 → web preview가 스테이징 참조"로 시작, 요청별 통합 preview는 후순위. 4순위 — 프롬프트 주입. 완화: 비밀 배치 기준(§10), read-only 레인, 비가역 동작의 사람 게이트, main push 금지.
...
**16-1. 관리자 인증 — GitHub OAuth.** 관리자는 GitHub OAuth로 로그인하고, `ADMIN_GITHUB_LOGINS` 허용 목록에 있는 계정만 통과시킨다(조직 멤버십 조회는 후순위). 첫 로그인 시 `users` 행을 `kind='admin'`으로 생성한다. 클라이언트는 §10의 초대링크+PIN 그대로이며, 두 인증은 별개 세션 쿠키를 쓴다. 어차피 GitHub App에 의존하는 구조라 외부 의존이 늘지 않는다.
...
| 레인 | cwd | 도구 | 모델 |
|---|---|---|---|
| chat (탐색·접수·견적) | 허브 루트 | `--tools Read Grep Glob` 화이트리스트 | Sonnet |
| work (구현·온보딩) | `work/{REQ}/` | 쓰기 포함 전체 | Opus |
...
**슬라이스 2 — 관리자 결정 큐.** GitHub OAuth, 검토 큐 테이블, 견적 조정·확정, 에스컬레이션 표시.
