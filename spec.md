...
│   ├── web/                     #   Next.js — systemd: ticketree-web (:3832)
...
포함: 프로젝트 1~2개(실 클라이언트), 접수 대화(버퍼링·멀티 라운드·탈출구), 티켓 테이블·스레드·명세 화면, 관리자 검토 큐·Spec diff 검토·견적 조정, 탐색/견적/구현 job + 큐·락·가드레일, 허브 repo·worktree, 수동 알림 뱃지, 온보딩 반자동(스크립트 + 체크리스트).

제외(후순위): 자동 알림 채널, 요청별 통합 preview, 프로젝트별 리눅스 유저·컨테이너 격리, 견적 보정 모델(데이터만 축적), 정산·청구서, 온보딩 완전 자동화.

검증 순서: 실 클라이언트 프로젝트로 전체 파이프라인 1바퀴 → 요청 10건 처리하며 견적 정확도·탐색 속도·에스컬레이션 빈도 측정.
...
**슬라이스 4 — 구현과 배포.** work/ worktree, 구현 job, 코드 PR, 배포 승인, manual 어댑터.
