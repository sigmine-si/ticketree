...
- `src/status.ts` — 상태 머신. 내부 status(12개)와 클라이언트 stage(4개)의 분리,
  전이 허용표, "지금 클라이언트 차례인가" 판정.
...
- `src/project-settings.ts` — 프로젝트마다 다른 값. 지금은 구현 job이 커밋할 수 있는
  경로(`codePaths`)뿐이다.
- `src/agent-io.ts` — 에이전트 출력 계약(zod). 접수·견적·구현 결과 JSON의 유일한 정의.
...
- `src/jobs/estimation.ts` — 확정 견적 산출.
...
- `src/app/requests/` — 클라이언트 포털. 요청 목록과 요청 상세(대화·견적·승인).
- `src/app/spec/` — 클라이언트가 보는 서비스 명세 화면.
