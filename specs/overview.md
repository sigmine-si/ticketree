...
숫자는 전부 실측에서 나왔다. 목표가 아니라 **지금 지키고 있는 선**이고, 넘기면
회귀로 본다.

| 무엇 | 기준 | 근거 |
|---|---|---|
| 대화 한 라운드 응답 | 90초 이내 (중앙값 40초) | 실측 13.7~105.7초. 소요는 출력 토큰 ÷ 70에 비례한다 |
| 화면에 첫 진행 문구 | 5초 이내 | 실측 3.6초. 이게 늦으면 멈춘 것으로 보인다 |
| 구현 job | 15분 이내 | 실측 63~573초. 넘으면 레인 타임아웃(45분)까지 가기 전에 사람이 본다 |
| 요청 한 건의 AI 원가 | 1만원 이내(환산) | 실측 ₩591~12,789. 넘으면 견적 대비 원가율을 다시 본다 |
| 러너 끊김 감지 | 90초 | 하트비트 20초 × 4.5. 이 안에 못 잡으면 요청이 멈춘 채로 남는다 |
| job 실패 | 1회 자동 재시도 후 사람에게 | 두 번 실패하면 대개 코드나 환경 문제다 |

**동시 실행.** 한 프로젝트의 같은 레인에서는 job이 하나만 돈다(advisory lock).
러너 프로세스 전체 동시 실행은 `RUNNER_MAX_CONCURRENT`(기본 2)로 막는다.
이 둘이 없으면 같은 워크스페이스에서 두 job이 브랜치를 다투게 된다.

**레인별 타임아웃.** 대화 5분 · 명세 10분 · 구현 45분. 넘으면 러너가 프로세스를 죽인다.

**보관.** 모든 상태 전이·job 계측·에이전트 원본 스트림을 지우지 않는다. 견적 보정의
데이터 자산이자 감사 추적이다. 알림도 지우지 않고 읽음 표시만 한다.

**가용성.** 러너가 죽어도 상태는 DB에 있으므로 다시 띄우면 이어서 돈다. 웹이 죽어도
진행 중인 job은 계속 돈다 — 둘은 직접 통신하지 않는다.
...
- v1.2 2026-07-24 지켜야 할 수준(비기능) 추가
